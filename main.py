			...
			if (des1 =="1"):
				...
				if cOp =="1":
					...
					if (des == "1"):
						...
						for amigo in p.amigos:
							#Lo que han visitado sus amigos
				...
				if (cOp=="1"):
					...
					l = graph.run("MATCH (n:Ciudad) WHERE n.nombre = '"+lugarN+"' RETURN n").data()
					if (len(l)>0):
						c = Ciudad()
						c.nombre=lugarN
						graph.pull(c)
					...
					lugarN = input("¿En donde has estado recientemente?\n")
					c = graph.run("MATCH (n:Pais) WHERE n.nombre = '"+lugarN+"' RETURN n").data()
					if (len(l)>0):
						c = Pais()
						c.nombre=lugarN
						graph.pull(c)
				...
				amigo = Persona()
				amigo.nombre=n0
				l = graph.run("MATCH (n:Persona) WHERE n.nombre = '"+n0+"' RETURN n").data()
				if (len(l)>0):
					graph.pull(amigo)
